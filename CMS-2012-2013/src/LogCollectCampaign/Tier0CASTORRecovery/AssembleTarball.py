...
def doLogCollection(runNumber, idx, stagedFiles, month):
    try:
        ...
        logCollectTarball = 'Run%d-LogCollect-%s-Amendment-%d.tar' % (runNumber, unique, idx)
        ...
        command = 'xrdcp -s %s root://castorcms//castor/cern.ch/cms/store/logs/prod/2012/%s/prodAgent/%s?svcClass=t0export' % (logCollectTarball, month, logCollectTarball)
