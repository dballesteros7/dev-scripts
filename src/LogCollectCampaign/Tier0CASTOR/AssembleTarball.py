...
def doLogCollection(runNumber, idx, stagedFiles, month):
    try:
        ...
        print "Starting stage out of the tarball to root://castorcms//castor/cern.ch/cms/store/logs/prod/2011/%s/prodAgent/%s" % (month, logCollectTarball)
        command = 'xrdcp -s %s root://castorcms//castor/cern.ch/cms/store/logs/prod/2011/%s/prodAgent/%s?svcClass=t0export' % (logCollectTarball, month, logCollectTarball)
        ...
        if rc == 0:
            print "Staged tarball to: root://castorcms//castor/cern.ch/cms/store/logs/prod/2011/%s/prodAgent/%s" %(month, logCollectTarball)
...
def main():
    ...
    try:
        ...
        for logArch in inputFileList:
            try:
                logArch = logArch[:-1]
                print "Staging: root://castorcms//%s" % logArch
                command = 'xrdcp -s root://castorcms//%s?svcClass=cmsprodlogs ./LogArchs/' % logArch
                args = shlex.split(command)
        ...
        for tarball in archivedTarballs:
            outputFile.write('/castor/cern.ch/cms/store/logs/prod/2011/%s/prodAgent/%s\n' % (month, tarball))
        outputFile.close()
    except Exception, ex:
        print str(ex)
        print traceback.format_exc()
